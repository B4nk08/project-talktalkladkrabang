...
async function createOtp({ user_id = null, otp_code, purpose, expires_at, meta = null }) {
...
async function findValidOtp({ user_id = null, otp_code, purpose }) {
  const query = user_id
    ? `SELECT * FROM otp_tokens WHERE user_id = ? AND otp_code = ? AND purpose = ? AND used = 0 AND expires_at > NOW() ORDER BY id DESC LIMIT 1`
    : `SELECT * FROM otp_tokens WHERE user_id IS NULL AND otp_code = ? AND purpose = ? AND used = 0 AND expires_at > NOW() ORDER BY id DESC LIMIT 1`;

  const params = user_id ? [user_id, otp_code, purpose] : [otp_code, purpose];
